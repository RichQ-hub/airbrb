...
import { StarRatingInputBtn, StarRatingInputContainer } from './styles';
...
const StarRatingInput = (props: StarRatingInputProps) => {
  const { maxRating, selectedRating, handleChangeRating } = props;
    ...
    <StarRatingInputContainer>
      {[...Array(maxRating)].map((_, idx) => {
        return (
          <StarRatingInputBtn
            key={idx}
            hovered={idx + 1 <= (hoveredRating || selectedRating)}
            type='button'
            onClick={() => {
